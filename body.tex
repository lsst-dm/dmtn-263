...
Vera C. Rubin Observatory is required to publish the observing schedule nightly.
Following our ``VO-first'' policy, we choose IVOA ObsLocTAP \citep{2021ivoa.spec.0724S} \footnote{\url{https://www.ivoa.net/documents/ObsLocTAP/}} as the protocol.
...
\citeds{LSE-30} (OSS) requires publication of schedule in advance of observing,
in OSS-REQ-0378 (Advanced Publishing of Scheduler Sequence):

\begin{quote}
\textbf{Specification}: The nominal survey schedule shall be published at least
\textbf{schedSeqAdvance} (24 hours)
followed by an updated schedule at least
\textbf{schedSeqDuration} (2 hours)
in advance of each observing visit.
\end{quote}

\citeds{LSE-72} (OCS-DM ICD) requires notification to DM of the ``next visit'',
in OCS-DM-COM-ICD-0031 (Advance Notice of Pointings):

\begin{quote}
\textbf{Specification}: Advance notice of telescope pointings for science data
acquisition shall be made available to Data Management as an OCS event,
no later than time
\textbf{pointingAdvanceNoticeTime} (20 seconds) before the start of the first
exposure of a standard visit or the only exposure of an alternate science visit.
\end{quote}

\citeds{LSE-61} (DMSR) flows down OSS-REQ-0378
to DMS-REQ-0353 (Publishing predicted visit schedule, priority 1b):

\begin{quote}
\textbf{Specification}: A service shall be provided to publish to the community
the next visit location and the predicted visit schedule provided by the OCS.
This service shall consist of both a web page for human inspection and a web API
to allow automated tools to respond promptly.

\textbf{Discussion}: The next visit and advanced schedule do not need to be
published using the same service or protocol.
\end{quote}

\citeds{DMTN-199} (Rubin Observatory Data Security Standards Implementation)
documents that the 24-hour requirement, which was added late
in the development of the project, derived from a government agency request.
...
THe full scheduler schema is at \url{https://rubin-scheduler.lsst.io/fbs-output-schema.html}.
...
The 24 hour schedule is available at USDF and could be picked up independently.
The updates could then be handled by looking at two Kafka events namely
\texttt{ lsst.sal.Scheduler.logevent\_predictedSchedule} which gives then next 2 hours or so of events and
\texttt{ lsst.sal.Scheduler.logevent\_nextVisit}  which gives the next pointing (almost certain to happen).

While we are required to give 24 hours notice,
...
The 24 hour schedule can be input with priority 2 (denoting lower likelihood of observation).
We can use \texttt{observationStartMJD} and \texttt{visitExposureTime} as \texttt{t\_min,t\_max} addign a buffer:
\begin{equation}
 \begin{aligned}
	buffer &= 10 m \\
	t_{min} &= observationStartMJD - buffer \\
	t_{max} &= observationStartMJD + visitExposureTime + buffer\\
 \end{aligned}
\end{equation}
As some of the new pointings will replace existing ones, the existing entries should be marked \texttt{unscheduled}.
...
Observations in the \texttt{predictedSchedule} would be marked with priority 1.
Observations in the \texttt{nextVisit} would be marked with priority 0.


\subsection{History of observations}
Next we need to record which observations were made to fill the \texttt{ObsDataSet}.
There is currently no ID in the scheduler forecast so there is no explicit matching of observation to forecast.
However as of January 2025 the Scheduler will start to include \texttt{target\_id} which should propagate through the messages allow easier association of predicted and actual observations.
